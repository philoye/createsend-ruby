require File.dirname(__FILE__) + '/helper'

class TransactionalBasicEmailTest < Test::Unit::TestCase
  multiple_contexts "authenticated_using_oauth_context", "authenticated_using_api_key_context" do
    setup do
      @client_id = "87y8d7qyw8d7yq8w7ydwqwd"
      @email = CreateSend::Transactional::BasicEmail.new @auth, @client_id
    end

    should "send basic email to one recipient" do
      stub_post(@auth, "transactional/basicemail/send", "tx_send_single.json")
      email = {
        "From"       => "George <george@example.com>",
        "Subject"    => "Thanks for signing up to Vandelay Industries",
        "To"         => "Bob Sacamano <bob@example.com>",
        "HTML"       => "<h1>Welcome</h1><a href='http://example.com/'>Click here</a>",
        "BasicGroup" => 'Ruby test group'
      }
      response = CreateSend::Transactional::BasicEmail.new(@auth).send(email)
      response.length.should == 1
      response[0].Recipient.should == "\"Bob Sacamano\" <bob@example.com>"
    end

    should "send basic email to with all the options" do
      stub_post(@auth, "transactional/basicemail/send", "tx_send_multiple.json")
      email = {
        "From" => "T-Bone <tbone@example.com>",
        "ReplyTo" => "george@example.com",
        "Subject" => "Thanks for signing up to Vandelay Industries",
        "To" => [
          "Bob Sacamano <bob@example.com>",
          "Newman <newman@example.com>",
        ],
        "CC" => [],
        "BCC" => [],
        "HTML" => "<h1>Welcome</h1><a href='http://example.com/'>Click here</a>",
        "Text" => "Instead of using the auto-generated text from the HTML, you can supply your own.",
        "Attachments" => [
          "Name" => "filename.gif",
          "Type" => "image/gif",
          "Content" => ""
        ],
        "BasicGroup" => "Ruby test group",
        "AddRecipientsToListID" => "6d0366fcee146ab9bdaf3247446bbfdd"
      }
      response = CreateSend::Transactional::BasicEmail.new(@auth).send(email)
      response.length.should == 2
      response[1].Recipient.should == "\"Newman\" <newman@example.com>"
    end

    should "get the list of basic groups" do
      stub_get(@auth, "transactional/basicemail/groups", "tx_basicemail_groups.json")
      response = CreateSend::Transactional::BasicEmail.groups(@auth)
      response.length.should == 3
      response[0].BasicGroup.should == "Password Reset"
    end

  end
end

