...
class TransactionalTimelineTest < Test::Unit::TestCase
  multiple_contexts "authenticated_using_oauth_context", "authenticated_using_api_key_context" do
    ...
    should "get statistics with the default parameters" do
      stub_get(@auth, "transactional/statistics", "tx_statistics_basic.json")
      response = CreateSend::Transactional::Timeline.new(@auth).statistics
      response.Sent.should == 1000
      response.Opened.should == 300
    end

    should "get statistics filtered by date and basic group" do
      stub_get(@auth, "transactional/statistics?from=2015-01-01&to=2015-06-30&timezone=client&basicGroup=Password%20Reset", "tx_statistics_basic.json")
      response = CreateSend::Transactional::Timeline.new(@auth).statistics(
        "from" => "2015-01-01",
        "to" => "2015-06-30",
        "timezone" => "client",
      ...
      response.Query.BasicGroup.should == "Password Reset"
      response.Sent.should == 1000
    end
    ...
    should "get the message timeline for a basic group" do
      stub_get(@auth, "transactional/messages?status=all&count=200&sentBeforeID=#{@before_id}&sentAfterID=#{@after_id}&basicGroup=Password%20Reset&clientID=#{@client_id}", "tx_messages_basic.json")
      response = CreateSend::Transactional::Timeline.new(@auth).messages(
        "status" => 'all',
        "count" => 200,
        "sentBeforeID" => @before_id,
